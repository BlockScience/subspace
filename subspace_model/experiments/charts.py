...
def negative_profits(profit1_timestep_df: pd.DataFrame) -> hv.element.chart.Curve:
    chart = profit1_timestep_df.loc[:, (profit1_timestep_df < 0).any()].reset_index().hvplot.line(x='days_passed', logy=True, width=1000, height=500, title='Negative Profits')
    return chart
...
def circulating_supply_volatility(sim_df: pd.DataFrame, experiment: str) -> hv.core.overlay.NdOverlay:
    s = window_volatility(sim_df.set_index('days_passed').circulating_supply.diff()).reset_index()

    chart = s.hvplot.line(x='days_passed', 
            y='circulating_supply',
            title='Circulating Supply Volatility (z-score on weekly standard deviation)', width=1000, height=500)

    return chart

def weekly_rewards_to_nominators(sim_df: pd.DataFrame, experiment: str) -> hv.core.overlay.NdOverlay:
    n_days=7
    ...
    chart = s.hvplot.line(title="Weekly Rewards to Nominators (% of Circ Supply per day)", ylabel='Weekly Rewards to Nominators (% of Circulating Supply)', height=500, width=1000, yformatter='%.2f%%')

    return chart

def weekly_issuance_rate(sim_df: pd.DataFrame, experiment: str) -> hv.core.overlay.NdOverlay:
    n = 7
    fig_df = sim_df.set_index('days_passed')
    s = fig_df.reward_issuance_balance.diff() * -1.0 
    s /= fig_df.max_credit_supply
    s = s.rolling(n).sum()[n-1::n]
    chart = s.hvplot(x='days_passed', y='reward_issuance_balance', title="Weekly Reward Issuance Rate", ylabel='% of Max Credit Supply ', yformatter='%.8f%%', height=500, width=1000)
    return chart

def cumulative_issuance_rate(sim_df: pd.DataFrame, experiment: str) -> hv.core.overlay.NdOverlay:
    n = 7
    fig_df = sim_df.set_index('days_passed')
    s = fig_df.reward_issuance_balance.diff() * -1.0
    s /= fig_df.max_credit_supply
    ...
    chart = s.hvplot(x='days_passed', y='reward_issuance_balance', title="Cumulative Reward Issuance Rate", ylabel='% of Max Credit Supply ', yformatter='%.8f%%', height=500, width=1000)
    return chart
